...
RefreshPMs::RefreshPMs() {
    finalValue = 0;
    firstTime=true;
    std::string serviceName="PM/PMA0/";
...
string RefreshPMs::processOutputMessage(string output) {
    ...
    while(output.length()>0&&count<maxCount&&count<services.size()){
        ...
        if(firstTime){
            ...
            if(services[count].rfind("PM/PMA0/TDC_12_PHASE_TUNING",0)==0){
            ...
            else if(services[count].rfind("PM/PMA0/TDC_3_PHASE_TUNING",0)==0){
                long long finalValue = 0;
                finalValue += stoll(value.substr(6,2), nullptr, 16)*13*8/7;
                updateTopicAnswer(services[count], std::to_string(finalValue));
            }
            else if(services[count].rfind("PM/PMA0/RAW_TDC_DATA",0)==0){
                std::stringstream ss;
                ss << std::hex << hexValue;
                updateTopicAnswer(services[count], "0x"+ss.str());
            }
            else if(services[count].rfind("PM/PMA0/ADC0_DISPERSION",0)==0||services[count].rfind("PM/PMA0/ADC1_DISPERSION",0)==0){
                updateTopicAnswer(services[count], std::to_string(std::sqrt(hexValue)));
            }
            else if(services[count].rfind("PM/PMA0/CFD_THRESHOLD",0)==0||services[count].rfind("PM/PMA0/CFD_ZERO",0)==0||services[count].rfind("PM/PMA0/ADC_ZERO",0)==0||services[count].rfind("PM/PMA0/ADC_DELAY",0)==0){
                //do sprawdzenia
                std::string returnStr = "";
            ...
            Print::PrintInfo(std::to_string(count));
            ...
            else if(services[count].rfind("ADC0_DISPERSION",0)==0||services[count].rfind("PM/PMA0/ADC1_DISPERSION",0)==0){
                updateTopicAnswer(services[count], std::to_string(std::sqrt(hexValue)));
            }
            else if(services[count].rfind("PM/PMA0/CFD_THRESHOLD",0)==0||services[count].rfind("PM/PMA0/CFD_ZERO",0)==0||services[count].rfind("PM/PMA0/ADC_ZERO",0)==0||services[count].rfind("PM/PMA0/ADC_DELAY",0)==0){
                std::string returnStr = "";
